...
//    - Finds ALL marker matches, ignores quest log rect (45,282)-(72,311) and overlay area
//    - Captures ROI below marker, OCRs the distance number (e.g. "76m")
//    - distance > 3m  : W + A/D steering (no Shift - player controls run/walk)
...
static double gPlaySpeed = 1.0; // multiplier: 1.0 = exact, 1.1 = 10% slower, 0.9 = 10% faster

// Precision sleep using QPC spin-wait for the final few ms.
...
static void precise_sleep_us(uint64_t us)
{
    ...
    uint64_t ticks = (uint64_t)((double)us * freq.QuadPart / 1e6);

    // Sleep for most of the duration (leave 2ms for spin-wait)
    if (us > 2500)
    {
...
// exactly over gQuestLogIgnore so you can see what area is being excluded.

static HWND gIgnoreRectHwnd = nullptr;

static LRESULT CALLBACK IgnoreRectProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
...
static HWND make_ignore_zone(const RECT &r, const char *label)
{
...
static void show_ignore_rect_overlay()
{
...
static LRESULT CALLBACK OverlayProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        if (qx >= 0)
        {
            ...
            snprintf(line, sizeof(line), "Quest: (%d,%d) dx=%d conf=%.2f  dist=%dm",
                     qx, qy, qx - screenCX, gQuestMarkerConf.load(), qd);
        }
        else
...
static std::atomic<bool> gQuestDebugSave{false}; // set true once to trigger a debug screenshot
                              ...
                              cv::Point &outCenter, double &outScore)
{
    ...
    // Debug screenshot: saves quest_debug.png showing all hits and ignore zones
    bool doDebug = gQuestDebugSave.exchange(false);
    if (doDebug)
    {
        cv::Mat dbg = screen.clone();
...
static void start_quest_walk(const cv::Mat &questTempl, double markerTh, int deadzonePx, int tickMs)
{
    ...
    gQuestWalkThread = std::thread([questTempl, markerTh, deadzonePx, tickMs]()
    {
        ...
        // W is NOT pressed at start - only pressed when marker is actively found
        ...
        while (gRunQuestWalk)
        {
            ...
            // --- D key = save debug screenshot on next detection tick ---
            if (GetAsyncKeyState(VK_F9) & 1)
            {
                gQuestDebugSave = true;
                std::puts("[QUEST] F9 pressed - debug screenshot will be saved as quest_debug.png");
            }
            ...
            if (!markerFound)
            {
                ...
                // Marker not visible - stop everything and wait
                release_move_keys();
                arrived = false;
                overlay_invalidate();
                Sleep(tickMs);
            ...
            std::printf("[QUEST] marker=(%d,%d) screenCenter=%d  dx=%d  conf=%.2f dist=%dm arrived=%d\n",
                        markerCenter.x, markerCenter.y, screen.cols/2,
                        markerCenter.x - screen.cols/2, conf, distM, (int)arrived);

        // ============================================================
        // Distance-based movement control - NO autorun
        // W/Shift are controlled by the player
        // Tool only handles: A/D steering + arrival stop
        // ============================================================
        ...
        {
            // Arrived - stay stopped until marker vanishes (handled above) or dist grows back
            if (distM >= 0 && distM >= kResumeMeters)
            {
                std::printf("[QUEST] Resuming - dist=%dm.\n", distM);
                arrived = false;
                // Fall through to steering below
            }
            else
            {
                release_move_keys();
...
static bool play_file(const char *path)
{
    ...
    for (size_t i = 0; i < events.size(); ++i)
    {
        ...
        if (e.t_us > prev_t)
            precise_sleep_us(e.t_us - prev_t);
        prev_t = e.t_us;
...
// ========================= Play full (pure automation) =========================
// Quest walk navigates toward the marker, hunt handles enemies.
// No macro replay - the recording is not used here.
                      ...
                      int ignL=45, int ignT=282, int ignR=72, int ignB=311)
{
    ...
    std::puts("[PLAYFULL] ESC - stopping.");
    gPlaying = false;
    stop_all_threads();
    timeEndPeriod(1);
